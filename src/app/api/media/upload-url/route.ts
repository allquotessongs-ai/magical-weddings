...
export async function POST(request: Request) {
  try {
    ...
    if (input.purpose === "gallery") {
      const { count } = await admin.from("wedding_media").select("id", { count: "exact", head: true }).eq("wedding_id", input.weddingId).eq("purpose", "gallery").eq("active", true);
    ...
    return NextResponse.json({ weddingId: input.weddingId, path, token: data.token, purpose: input.purpose, mimeType: input.mimeType, size: input.size, altText: input.altText, focalX: input.focalX, focalY: input.focalY });
  } catch (error) {
