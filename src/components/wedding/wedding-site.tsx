...
export function WeddingSiteRenderer({ wedding, preview = false }: { wedding: WeddingSite; preview?: boolean }) {
  ...
  return <div className={`wedding-site theme-${wedding.theme.id} radius-${wedding.theme.radius} shadow-${wedding.theme.shadow} spacing-${wedding.theme.sectionSpacing} motion-${wedding.theme.motion} images-${wedding.theme.imageTreatment} buttons-${wedding.theme.buttonStyle}`} style={css} data-opened={opened} data-theme={wedding.theme.id}>
    {wedding.media.music && <audio ref={audioRef} src={mediaUrl(wedding.media.music)} loop muted={muted} />}
    ...
    {wedding.media.music && wedding.entitlements.backgroundMusic && opened && <div className="audio-controls"><button aria-label={playing ? "Pause music" : "Play music"} onClick={toggleMusic}>{playing ? <Pause /> : <Music2 />}</button><button aria-label={muted ? "Unmute music" : "Mute music"} onClick={() => setMuted((value) => !value)}>{muted ? <VolumeX /> : <Volume1 />}</button><label><span className="sr-only">Music volume</span><input aria-label="Music volume" type="range" min="0" max="1" step="0.05" value={volume} onChange={(event) => { const value = Number(event.target.value); setVolume(value); if (audioRef.current) audioRef.current.volume = value; }} /></label></div>}
  </div>;
}
